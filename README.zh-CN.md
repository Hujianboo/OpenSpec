...
在仓库根目录执行 `blockspec init` 后，即可像上游 OpenSpec 一样使用 `/opsx:…` 斜杠命令。除本节说明外，下文仍按 OpenSpec 风格描述工作流与 CLI。
...
告诉 AI：用 **`/opsx:tdd <要做的事>`** 走测试驱动（含 `test-plan` 与 `[RED]/[GREEN]/[REFACTOR]` 纪律），或用 **`/opsx:propose <要做的事>`** 走更轻的默认流程。
