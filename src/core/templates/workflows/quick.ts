/**
 * Quick / Fast Lane workflow templates.
 */
import type { SkillTemplate, CommandTemplate } from '../types.js';

const QUICK_INSTRUCTIONS = `Fast Lane implementation for lightweight changes.
...
export function getQuickSkillTemplate(): SkillTemplate {
  return {
    name: 'openspec-quick',
    description: 'Fast Lane for small, low-risk changes. Use when the user wants one-step lightweight implementation with a minimal record.',
...
export function getOpsxDoCommandTemplate(): CommandTemplate {
  return {
    name: 'OPSX: Do',
    description: 'Fast Lane: implement a lightweight change immediately with a minimal record',
    category: 'Workflow',
    tags: ['workflow', 'quick', 'fast-lane'],
    content: QUICK_INSTRUCTIONS,
  };
}
