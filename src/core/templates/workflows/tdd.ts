 ...
 *
 * Skill and command templates for the TDD (test-driven) workflow.
 */
...
export function getTddSkillTemplate(): SkillTemplate {
  return {
    ...
    instructions: `Propose a new change using the **TDD workflow**.
...
export function getOpsxTddCommandTemplate(): CommandTemplate {
  return {
    name: 'OPSX: TDD',
    description: 'Propose a new change using TDD workflow - proposal → specs → test-plan → design → tasks',
    category: 'Workflow',
    tags: ['workflow', 'artifacts', 'tdd'],
    content: `Propose a new change using the **TDD workflow**.
...
**Input**: The argument after \`/opsx:tdd\` is the change name (kebab-case), OR a description of what the user wants to build.
